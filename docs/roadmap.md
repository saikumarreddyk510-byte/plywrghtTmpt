...
becomes a passing, reviewed spec. Everything _after_ a spec exists — running it
...
**✅ Complete.** Both shipped — see `docs/architecture.md` §4 and §11.
None of it requires Claude Code or Anthropic specifically: the pipeline is
also drivable from VS Code + GitHub Copilot (§12), and CI runs the suite and
uploads its reports with zero AI vendor configured — the AI jobs are optional
enrichment on top, not a dependency.

| #   | Enhancement                             | What it does                                                                                                                                                                                                                                                                                                                                          | Why it matters |
| --- | --------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- |
| 1   | ~~Self-healing locators~~               | ✅ **Shipped** — `docs/architecture.md` §4, the `heal-test` skill.                                                                                                                                                                                                                                                                                    | —              |
| 2   | ~~CI/CD wiring~~ — `.github/workflows/` | ✅ **Shipped** — `docs/architecture.md` §11 and [`cicd.md`](./cicd.md). Full suite on every push, smoke on PRs and daily, HTML reports uploaded as artifacts, `review-tests` auto-commenting on changed specs, plus `e2e-gate.yml` for use as a release gate. Needs `ANTHROPIC_API_KEY` only for the AI step — the test-execution job itself doesn't. | —              |
...
"100% AI-driven" should mean the _work_ is automated, not that every decision
...
- **Nothing auto-closes a bug as "flake"** without evidence attached
  (trace/screenshot/history) — that's exactly the failure mode that erodes
  trust in a test suite.
